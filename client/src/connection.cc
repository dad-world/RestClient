...
std::string RestClient::Connection::performCurlRequestGetToken(const std::string& uri)
{
  time_t now = time(0);
  char *dt = ctime(&now);
  cout << dt << "-------------------------------------" << endl;
  std::stringstream out;
  curl_easy_setopt(this->curlHandle, CURLOPT_WRITEFUNCTION, write_data);
  curl_easy_setopt(this->curlHandle, CURLOPT_WRITEDATA, &out);
  ...
  curl_easy_reset(this->curlHandle);
    ...
    //用jsoncpp解析
    string jsonContent = out.str();
    DealJson dealJson;
    return dealJson.get_token(jsonContent);
...
RestClient::Connection::performCurlRequest(const std::string& uri) {
  ...
  *//** set data object to pass to callback function *//*
  ...
  string jsonContent = out.str();
  DealJson dealJson;
  dealJson.deal_test(jsonContent);
...
string RestClient::Connection::get_token(const std::string& url,
                                    const std::string& data)
{
    /** Now specify we want to POST data */
    curl_easy_setopt(this->curlHandle, CURLOPT_POST, 1L);
    ...
    return this->performCurlRequestGetToken(url);
}
/**
